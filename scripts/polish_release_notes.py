#!/usr/bin/env python3
"""Polish raw git-cliff release notes into concise GitHub Release notes.

Reads release configuration from Cargo.toml (package name) and `.env`
(Anthropic API for AI rewrite). If the AI env is not configured, the raw
notes are passed through unchanged.

This mirrors the proxai release pipeline so fst can use the same workflow.
"""
...
def polish(raw_notes: str) -> str | None:
    ...
    package_name = (
        env_value("RELEASE_PACKAGE_NAME") or f"{load_cargo_name()}-<tag>-windows-x86_64"
    )
    ...
    prompt = f"""
Rewrite these structured release notes into concise GitHub Release notes.
