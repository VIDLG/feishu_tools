...
## 工具链：pixi + just，不裸跑 cargo

`fst` 是纯 Rust 二进制，但 CI / 发布工具链（`just` / `git-cliff` / `lefthook` / `rtk-cli` / 发布用的 Python 脚本）需要可复现。`pixi` 通过 `pixi.lock` 锁住这套工具链；`Justfile` 把所有命令包装成 `pixi run -- rtk ...`。Rust 本身仍由 `rust-toolchain.toml` 锁定，因为它是社区标准，不应被 pixi 重复发明。

Windows 上从 MSYS bash 调 `just` 时会注入伪环境变量 `!::`，pixi 的激活脚本会绊到它，所以 `Justfile` 里定义了：
...
失败通过 `LarkError`（`src/error.rs`）映射到语义化退出码，让 shell 脚本可以根据 auth/scope/network/lark-cli 错误作出不同反应。新代码不要用 `anyhow::anyhow` —— 加个类型化变体，映射到某个退出码。

## 不要全局状态

子进程的 cwd 通过 `tokio::process::Command::current_dir` 传入。不要用 `env::set_current_dir`。测试和并发运行不能在进程状态上竞争。
